...
class RobotArmBluetoothService {
  ...
  Future<void> _setupRobotArmService() async {
    try {
      ...
      for (var s in service) {
        ...
        if (uuid.contains(
          BluetoothConstants.robotArmServiceUuid.toUpperCase(),
        )) {
          robotArmService = s;
          break;
        }
  ...
  Future<void> _subscribeToRobotArmData(BluetoothService service) async {
    try {
      ...
      await _robotArmDataSubscription?.cancel();
      _robotArmDataSubscription = null;
      _robotArmDataSubscription = await _bluetoothService
          .subscribeToCharacteristic(charcteristic, _handleReceivedData);
  ...
  Future<void> writeData(RobotArmPayloadData data) async {
    try {
      if (connectedDevice == null || _writeCharacteristic == null) {
        debugPrint(
          'No device connected or write characteristic not found. Cannot write data.',
        );
        return;
      }
      ...
      final packetData = RobotArmControllerProtocol.createProtocolPacket(
        payloadData: data,
      );
  ...
  Future<void> scanForRobotArmDevices() async {
    try {
      await _bluetoothService.startScan(onDeviceFound: onDeviceFound);
    } catch (e) {
