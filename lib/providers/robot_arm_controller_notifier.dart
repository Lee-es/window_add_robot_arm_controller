...
class RobotArmControllerState {
  ...
  int get currentRunSpeed => runSpeed;
...
class RobotArmControllerNotifier extends _$RobotArmControllerNotifier {
  ...
  RobotArmControllerState build() {
    ...
    final subBleCurrentStatus = _bluetoothService.connectionStatusStream.listen((status){
      debugPrint('[Notifier] BLE 상태 변경: $status');
      state = state.copyWith(bleConnectionStatus: status);
    });

    final subBleReceivedData = _bluetoothService.robotArmDataStream.listen((data){
  ...
  Future<void> changeRunSpeed(int speed) async{
