...
class BleService {
  ...
  Future<List<BluetoothDevice>> getBondedDevices() async {
    ...
    if (Platform.isWindows) {
      debugPrint(
        '[BLE] Windows에서는 시스템 페어링 기기 목록 조회를 지원하지 않습니다. 스캔(Scan)을 사용해주세요.',
      );
      return [];
    }
  ...
  /// 특정 키워드로 페어링된 기기 필터링
  Future<List<BluetoothDevice>> getBondedDevicesByKeywords(
  ...
  Future<void> startScan({
    ...
    Duration timeout = const Duration(
      seconds: BluetoothConstants.scanTimeoutSeconds,
    ),
      ...
      _scanSubscription = FlutterBluePlus.scanResults.listen((results) {
        for (var result in results) {
          ...
          if (onDeviceFound != null) {
            final targetDeviceName = BluetoothConstants.armDeviceName
                .toUpperCase();
  ...
  Future<void> connect(
    ...
    try {
      ...
      final bool isWindowsAutoConnect = Platform.isWindows
          ? false
          : autoConnect;
